#!/usr/bin/env python3
"""Generate KRX portfolio report using agent-driven signals (90 LLM calls)."""
...
PREDICT_JSON = ROOT / "portfolios" / "krx_predict_20260428.json"
AGENT_DIR = Path("/tmp/agent_results")
OUTPUT_XLSX = ROOT / "portfolios" / "krx_20260428_buffett_fisher_lynch_agents.xlsx"
OUTPUT_TXT = ROOT / "portfolios" / "krx_20260428_buffett_fisher_lynch_agents.txt"
TOP_N = 30
INDEX_NAME = "krx"
...
INVESTOR_LABELS = {"buffett": "Warren Buffett", "lynch": "Peter Lynch", "fisher": "Phil Fisher"}
...
CAP_LABELS = {"mega": "메가캡", "large": "대형주", "mid": "중형주", "small": "소형주", None: "시총 N/A", "cash": "현금"}
...
def load_agent_results():
    return {inv: json.loads((AGENT_DIR / f"{inv}_20260428.json").read_text(encoding="utf-8")) for inv in INVESTORS}


def build_rows(data, agents):
    rows = []
    for item in data["rankings"][:TOP_N]:
        ...
        for investor in INVESTORS:
            entry = agents[investor].get(item["ticker"])
            if not entry:
                continue
            sig = entry["signal"]
            ...
            if sig == "bullish":
                ...
                bearish_count += 1
            analyses[investor] = {"signal": sig, "confidence": conf, "reasoning": entry["reasoning"]}
        ...
        row = {
            **item,
            "market_cap": {"value": cap_value, "display": cap_display, "category": cap_category},
...
def create_text_report(data, rows, portfolio, cash_weight):
    lines = []
    ...
    lines += [
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "📋 AI Hedge Fund 포트폴리오 리포트 (90 LLM 에이전트 기반)",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        f"분석 일자    : {data['analysis_date']}",
        f"분석 대상    : KRX 상위 {TOP_N}개 종목",
        "분석 전략    : 하이브리드 (펀더멘털 70% + 모멘텀 30%)",
        f"투자자 관점  : {investor_names} (각 30개 종목 LLM 에이전트 호출)",
        "데이터 소스  : DART + PyKRX + KRX Open API (predict) + Task 에이전트 30×3=90 호출",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "📊 포트폴리오 구성",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "#   종목                     비중     시총       종합신호       신뢰도   예상수익률   ROE      합의",
        "─────────────────────────────────────────────────────────────────────────────────────────────",
    ]
    ...
    lines += [
        "",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "💬 투자자별 핵심 분석 근거 (편입 상위 5개 종목, LLM 에이전트 reasoning)",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
    ]
    ...
    lines += [
        ...
        f"현금/대기자금        : {cash_weight:.1f}% (15% 단일, 35% 분류 캡 후 잔여)",
        f"평균 신뢰도          : {avg_conf}%",
        ...
        "",
        "투자자 합의 분포:",
        f"  만장일치 (3/3)     : {len(unanimous)}개" + (" — " + ", ".join(row["ticker"] for row in unanimous) if unanimous else ""),
        f"  다수 합의 (2/3)    : {len(majority)}개" + (" — " + ", ".join(row["ticker"] for row in majority[:8]) + (f" 외 {len(majority)-8}개" if len(majority) > 8 else "") if majority else ""),
        "",
        "시가총액 분포:",
    ]
    ...
    lines += [
        ...
        f"  최대 비현금 분류 비중: {CAP_LABELS[max_cap_category]} {max_cap_weight:.1f}% (제한 35% 내)" if non_cash_weights else "  최대 비현금 분류 비중: N/A",
        "",
        "투자 철학 불일치 경고 (predict 산출):",
    ]
    if warnings:
    ...
    lines += [
        ...
        "   predict: DART + PyKRX + KRX Open API | investor-analysis: LLM 에이전트 30×3=90 호출",
...
def create_excel(data, rows, portfolio, cash_weight):
    wb = Workbook()
    ...
    ws1["A1"] = "AI Hedge Fund 포트폴리오 리포트 (90 LLM 에이전트)"
    ...
    summary = [
        ...
        (7, "분석 방식", "LLM 에이전트 30종목 × 3투자자 = 90 호출"),
        (9, "포트폴리오 통계", ""),
        (10, "편입 종목 수", f'=COUNTA(\'포트폴리오\'!B2:B{stock_last})&" / {TOP_N} ("&TEXT(COUNTA(\'포트폴리오\'!B2:B{stock_last})/{TOP_N},"0.0%")&")"'),
        (11, "평균 신뢰도", f"=AVERAGE('포트폴리오'!F2:F{stock_last})/100"),
        (12, "평균 예상 수익률", f"=AVERAGE('포트폴리오'!G2:G{stock_last})"),
        (13, "강력매수 비중", f'=SUMIF(\'포트폴리오\'!E2:E{stock_last},"강력매수",\'포트폴리오\'!D2:D{stock_last})'),
        (14, "매수 비중", f'=SUMIF(\'포트폴리오\'!E2:E{stock_last},"매수",\'포트폴리오\'!D2:D{stock_last})'),
        (15, "현금/대기자금", cash_weight / 100),
        (17, "시가총액 분포", ""),
    ]
    for row, label, value in summary:
        ws1.cell(row=row, column=1, value=label)
        ws1.cell(row=row, column=2, value=value)
        ws1.cell(row=row, column=1).font = Font(name="맑은 고딕", bold=row in {9, 17})
    ...
    for idx, row in enumerate(rows, 2):
        ...
        values = [
            row["rank"], row["ticker"], row["company_name"], row["total_score"],
            scores.get("fundamental"), scores.get("enhanced_momentum"),
            row.get("ensemble_score"), row.get("signal"),
            (row.get("predicted_return_1y") or 0) / 100,
            row["market_cap"]["display"],
            metrics.get("pe") if metrics.get("pe") is not None else "N/A",
    ...
    detail_headers = ["종목코드", "회사명", "투자자", "신호", "신뢰도", "분석근거 (LLM 에이전트)"]
    ...
    for row in rows:
        for inv in INVESTORS:
            analysis = row["investor_analysis"][inv]
            values = [row["ticker"], row["company_name"], INVESTOR_LABELS[inv],
                      SIGNAL_KR[analysis["signal"]], analysis["confidence"], analysis["reasoning"]]
            for col, value in enumerate(values, 1):
    ...
    ws5.column_dimensions["F"].width = 110
    ...
    risk_items = [
        ...
        (14, "투자자 합의 품질 (LLM)", ""),
        (15, "만장일치 (3/3) 비율", f'=COUNTIF(\'포트폴리오\'!L2:L{stock_last},"3/3")/COUNTA(\'포트폴리오\'!B2:B{stock_last})'),
        (16, "다수합의 (2/3) 종목 수", sum(1 for r in rows if r["bullish_count"] == 2 and r["included"])),
        (18, "비편입 사유 분포", ""),
...
def main():
    ...
    agents = load_agent_results()
    rows = build_rows(data, agents)
