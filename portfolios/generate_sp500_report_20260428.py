#!/usr/bin/env python3
"""Generate S&P 500 portfolio report from the latest predict JSON."""
...
PREDICT_JSON = Path("/tmp/ai_hedge_fund_sp500_predict_20260428.json")
...
def reasoning_for(investor: str, row: dict, inv_signal: str) -> str:
    ...
    if investor == "buffett":
        if inv_signal == "bullish":
            return f"ROE {fmt_pct(roe)}와 품질 지표가 강해 경제적 해자 관점에서 우호적입니다. P/E {fmt_num(pe)} 기준 안전마진은 별도 확인이 필요합니다."
    ...
    if investor == "lynch":
        if inv_signal == "bullish":
            return f"매출 성장 {fmt_pct(growth)}과 성장 스토리가 뚜렷해 GARP 후보입니다. PEG {fmt_num(peg)} 기준 가격 대비 성장 매력도도 함께 확인됩니다."
        if inv_signal == "bearish":
...
def build_rows(data: dict) -> list[dict]:
    ...
    for item in data["rankings"][:TOP_N]:
        market_cap = item.get("market_cap") or {}
        cap_value = market_cap.get("value")
        cap_category = market_cap.get("category")
        cap_display = market_cap.get("display") or "N/A"
...
def create_text_report(data: dict, rows: list[dict], portfolio: list[dict], cash_weight: float) -> str:
    ...
    lines += [
        ...
        f"분석 대상    : S&P 500 상위 {TOP_N}개 종목",
        "분석 전략    : 하이브리드 (펀더멘털 70% + 모멘텀 30%)",
        f"투자자 관점  : {investor_names}",
        "데이터 소스  : Yahoo Finance (predict) + predict 투자자 점수 기반 정성 분석",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "",
        "══════════════════════════════════════════════════════════════════════════════════════════════",
        "📊 포트폴리오 구성",
    ...
    lines += [
        ...
        "   predict: Yahoo Finance | investor-analysis: predict 투자자 점수 기반 정성 분석",
