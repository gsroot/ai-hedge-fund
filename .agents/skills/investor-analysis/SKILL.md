...
- Yahoo 재무·시가총액·뉴스·내부자 API는 현재 스냅샷이다. 과거 기준일의 미국 종목
  분석에는 사용하지 않으며, CLI도 기술적 가격 분석 외 요청을 차단한다.
- 과거 미국 종목의 펀더멘털 관점이 필요하면 실제 point-in-time 데이터 공급자를 먼저
  연결한다. 현재 값을 과거 날짜의 값처럼 설명하지 않는다.
- 한국 DART 재무자료는 기준일 당시 공시 가능했던 최근 연간 보고서만 사용한다.
- 한국 뉴스는 Naver Search API와 네이버 증권 폴백 모두 기준일 이후 기사를 제외하고,
  발행일을 검증할 수 없는 기사도 과거 분석에 사용하지 않는다.
- 모든 결과에 기준일, 사용한 소스, 누락 데이터, 현재 스냅샷 사용 여부를 명시한다.
- 데이터가 부족하면 confidence를 낮추는 데 그치지 말고 `insufficient_data`를 근거에 적는다.
...
투자자별 휴리스틱 가중치 (성과로 캘리브레이션되지 않았으므로 민감도 분석 필요):
